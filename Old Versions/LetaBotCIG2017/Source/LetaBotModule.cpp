...
void LetaBotModule::onStart()
{
	...
	*/
	...
	Broodwar->sendText("LetaBot version 0.31 , CIG version. Build orders added. No MCTS");
	...
	*/
	...
	MarineRush = new MarineRushManager();
...
void LetaBotModule::onFrame()
{
	...
	*/
	...
	if( WAMan != NULL && CurrentStrategy == Worker_Rush){
		WAMan->OnFrame();
	}

	if( MarineRush!= NULL && CurrentStrategy == BBS_Rush ){
		MarineRush->OnFrame();
	}
