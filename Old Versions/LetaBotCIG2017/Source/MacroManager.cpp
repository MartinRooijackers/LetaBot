...
void MacroManager::OnFrameProtossSquadDefenceExpansion(int id){
	...
	*/
	...
	for(unsigned int i=0; i<Squads[id].Marines.size(); i++){
		...
		std::set<Unit*> EnemyClose = Squads[id].Marines[i]->getUnitsInRadius( 20*32 );
		Unit* closest = NULL;
	...
	for(unsigned int i=0; i<Squads[id].FireBats.size(); i++){
		...
		std::set<Unit*> EnemyClose = Squads[id].FireBats[i]->getUnitsInRadius( 20*32 );
		Unit* closest = NULL;
		int closeDist = 9999999;
		bool PhotonCan = false;
		int totalPhoton = 0;
	...
	for(unsigned int i=0; i< Squads[id].Medics.size(); i++){
		...
		std::set<Unit*> UnitsClose = Squads[id].Medics[i]->getUnitsInRadius( 9*32 );
		Unit* closest = NULL;
		int closeDist = 9999999;
		bool PhotonCan = false;
		int totalPhoton = 0;
	...
	for(unsigned int i=0; i<0; i++){
		...
		*/
		...
		std::set<Unit*> EnemyClose = Squads[0].Tanks[i]->getUnitsInRadius( 15*32 );
		Unit* closest = NULL;
...
void MacroManager::OnFrameProtossSquadAttack(int id){
	...
	*/
	...
	BOOST_FOREACH( BWAPI::Unit* tank, Squads[0].Tanks ){
	  ...
	  //for(unsigned int i=0; i< Squads[0].ScienceVessels.size(); i++){
		...
		//std::set<Unit*> UnitsClose = scienceVessel->getUnitsInRadius( MaxDistAttack );
		Unit* closest = NULL;
		int closeDist = 9999999;
	...
	for(unsigned int i=0; i<Squads[0].Ghosts.size(); i++){
		...
		std::set<Unit*> EnemyClose = Squads[0].Ghosts[i]->getUnitsInRadius( 15*32 );
		Unit* closest = NULL;
		int closeDist = 9999999;
		Unit* closestLD = NULL;
	...
	*/
	...
	for(unsigned int i=0; i<Squads[0].Vultures.size(); i++){
		...
		std::set<Unit*> EnemyClose = Squads[0].Vultures[i]->getUnitsInRadius( 10*32 );
		Unit* closest = NULL;
		int closeDist = 9999999;
		bool PhotonCan = false;
		BOOST_FOREACH( Unit* close, EnemyClose )
	...
	for(unsigned int i=0; i<Squads[0].Marines.size(); i++){
		...
		//std::set<Unit*> EnemyClose = Squads[0].Marines[i]->getUnitsInRadius( 20*32 );
		Unit* closest = NULL;
		int closeDist = 9999999;
		bool PhotonCan = false;
		int totalPhoton = 0;
		...
		*/
		...
		if( Squads[0].Tanks.size() < 3  || BWTA::getRegion( Squads[0].Marines[i]->getPosition() ) == InfoMan->OurMainregion ){
			TankNearyBy = true;
		}
	...
	for(unsigned int i=0; i<Squads[0].FireBats.size(); i++){
		...
		int MaxDistAttack = 20*32;
		//std::set<Unit*> EnemyClose = Squads[0].FireBats[i]->getUnitsInRadius( 20*32 );
		Unit* closest = NULL;
		int closeDist = 9999999;
		bool PhotonCan = false;
		int totalPhoton = 0;
		...
		*/
		...
		if( Squads[0].Tanks.size() < 3 ||  closeDist < 2*32
			|| BWTA::getRegion( Squads[0].FireBats[i]->getPosition() ) == InfoMan->OurMainregion  ){
			TankNearyBy = true;
		}
	...
	for(unsigned int i=0; i< Squads[0].Medics.size(); i++){
		...
		std::set<Unit*> UnitsClose = Squads[0].Medics[i]->getUnitsInRadius( 15*32 );
		Unit* closest = NULL;
		int closeDist = 9999999;
		bool PhotonCan = false;
		int totalPhoton = 0;
		...
		*/
		...
		if( Squads[0].Tanks.size() < 3 || BWTA::getRegion( Squads[0].Medics[i]->getPosition() ) == InfoMan->OurMainregion ){
			TankNearyBy = true;
		}
	  ...
	  Unit* needsHeal = NULL;
		...
		std::set<Unit*> UnitsClose = Squads[0].SCVs[i]->getUnitsInRadius( MaxDistAttack );
		Unit* closest = NULL;
		int closeDist = 9999999;
		bool Sunken = false;
		int totSunk = 0;
	...
	for(unsigned int i=0; i<Squads[0].Tanks.size(); i++){
		...
		*/
		...
		int MaxDistAttack = 15*32;
		//std::set<Unit*> EnemyClose = Squads[0].Tanks[i]->getUnitsInRadius( MaxDistAttack );
		Unit* closest = NULL;
		Unit* closestDragoon = NULL;
		Unit* closestHT = NULL;
		Unit* closestReavu = NULL;
		...
		*/
		...
		if( closest == NULL ){
			...
			if( Broodwar->mapHash() == "aab66dbf9c85f85c47c219277e1e36181fe5f9fc" ){
				if( BWTA::getRegion( Squads[0].Tanks[i]->getPosition() ) == InfoMan->NatRegion ){
...
void MacroManager::OnFrameProtossSquadDefence(int id){
	...
	if( PState == P_Defend_Nat ){
		...
		BOOST_FOREACH( BWTA::Chokepoint* choke, NatReg->getChokepoints() ){
			if( choke->getRegions().first != InfoMan->OurMainregion
				&& choke->getRegions().second != InfoMan->OurMainregion){
					if(  BWTA::getGroundDistance( BWAPI::TilePosition( choke->getCenter() ) , InfoMan->OurBase  ) > FurthestChoke ){
					    FurthestChoke = BWTA::getGroundDistance( BWAPI::TilePosition( choke->getCenter() ) , InfoMan->OurBase  );
	...
	*/
	...
	for(unsigned int i=0; i< Squads[0].ScienceVessels.size(); i++){
		...
		//std::set<Unit*> UnitsClose = Squads[0].ScienceVessels[i]->getUnitsInRadius( 30*32 );
		Unit* closest = NULL;
		Unit* AMuta = NULL;
		int closeDist = 9999999;
		BOOST_FOREACH( Unit* close, EnemyUnits )
	...
	for(int i=0; i<Squads[id].Vultures.size(); i++){
		...
		//std::set<Unit*> EnemyClose = Squads[id].Vultures[i]->getUnitsInRadius( 30*32 );
		Unit* closest = NULL;
		int closeDist = 9999999;
		BOOST_FOREACH( Unit* close, EnemyUnits )
        {
		      ...
		      )
	  {
		...
		if( closest != NULL ){
			...
			if( (closest->getType() == BWAPI::UnitTypes::Protoss_Zealot 
				|| BWTA::getRegion( closest->getPosition() ) == InfoMan->OurMainregion  ) ){
   ...
   for(int i=0; i<Squads[id].Ghosts.size(); i++){
		...
		std::set<Unit*> EnemyClose = Squads[id].Ghosts[i]->getUnitsInRadius( 30*32 );
		Unit* closest = NULL;
		int closeDist = 9999999;
		BOOST_FOREACH( Unit* close, EnemyClose )
        {
	...
	for(int i=0; i<Squads[id].FireBats.size(); i++){
		...
		//std::set<Unit*> EnemyClose = Squads[id].FireBats[i]->getUnitsInRadius( 40*32 );
		Unit* closest = NULL;
		int closeDist = 9999999;
		BOOST_FOREACH( Unit* close, EnemyUnits )
        {
	...
	for(int i=0; i<Squads[id].Marines.size(); i++){
		...
		//std::set<Unit*> EnemyClose = Squads[id].Marines[i]->getUnitsInRadius( 40*32 );
		Unit* closest = NULL;
		int closeDist = 9999999;
		BOOST_FOREACH( Unit* close, EnemyUnits )
        {
	  ...
	  if( PState == P_Defend_Nat && Squads[0].Bunkers.size() > 0
		   && i < 4 ){
			   for( int b=0; b<Squads[0].Bunkers.size(); b++){
				   if(  BWTA::getRegion( Squads[0].Bunkers[b]->getPosition() ) != InfoMan->OurMainregion ){
	...
	//use stim before engaging enemy
		...
		if( CCmanager.size() > 1 ){
		  if( Squads[0].Marines[i]->getDistance( CCmanager[1]->CommandCenter ) > 12*32
			  &&  BWTA::getRegion( Squads[0].Marines[i]->getPosition() ) != InfoMan->OurMainregion
			  && Squads[0].Marines[i]->getDistance( InfoMan->PosMainChoke ) > 12*32 ){
			  FarAwayNat = true;
		  }
		}

		if( PState == P_Wall &&  BWTA::getRegion( Squads[0].Marines[i]->getPosition() ) != InfoMan->OurMainregion ){
			FarAwayNat = true;
		}
	...
	for(int i=0; i<Squads[id].Tanks.size(); i++){
		...
		//std::set<Unit*> EnemyClose = Squads[id].Tanks[i]->getUnitsInRadius( 40*32 );
		Unit* closest = NULL;
		int closeDist = 9999999;
		BOOST_FOREACH( Unit* close, EnemyUnits )
        {
	...
	for(unsigned int i=0; i< Squads[0].Medics.size(); i++){
		...
		std::set<Unit*> UnitsClose = Squads[0].Medics[i]->getUnitsInRadius( 30*32 );
		Unit* closest = NULL;
		int closeDist = 9999999;
		bool PhotonCan = false;
		BOOST_FOREACH( Unit* close, UnitsClose )
...
void MacroManager::OnFrameZergSquadAttack(int id){
	...
	*/
	...
	for(unsigned int i=0; i<Squads[0].Marines.size(); i++){
		...
		std::set<Unit*> EnemyClose = Squads[0].Marines[i]->getUnitsInRadius( MaxDistAttack );
		Unit* closest = NULL;
		int closeDist = 9999999;
		bool Sunken = false;
		int totSunk = 0;
		...
		if( Squads[0].Marines.size() > TotalLurker*3 ||  LurkerCloseToTank == true ){
			OverrunLurker = true;
		}
	  ...
	  // if we can't shoot, run away  if(kite)
		...
		if ( ( LanLatency == true ||  isUnderDS ) && (   ( closeDist < 4*32 &&  Squads[0].Marines[i]->getGroundWeaponCooldown() != 0 && LanLatency == true )
...
/*
	...
	for(unsigned int i=0; i<Squads[0].Goliaths.size(); i++){
		...
		std::set<Unit*> EnemyClose = Squads[0].Goliaths[i]->getUnitsInRadius( 11*32 );
		Unit* closest = NULL;
		int closeDist = 9999999;
		bool Sunken = false;
		int totSunk = 0;
			...
			||  isUnderDS )
	  {
		...
		//Broodwar->printf("%s",Marines[i]->getOrder().c_str() );
		int MaxDistAttack = 10*32;
		std::set<Unit*> UnitsClose = Squads[0].SCVs[i]->getUnitsInRadius( 10*32 );
		Unit* closest = NULL;
		int closeDist = 9999999;
		bool Sunken = false;
		int totSunk = 0;
	  ...
	  */
	  ...
	  for(unsigned int i=0; i< Squads[0].ScienceVessels.size(); i++){
		  ...
		  int MaxDistAttack = 15*32;
		std::set<Unit*> UnitsClose = Squads[0].ScienceVessels[i]->getUnitsInRadius( MaxDistAttack );
		Unit* closest = NULL;
		int closeDist = 9999999;
		...
		*/
		...
		if( closestRangeAttack == NULL && closeDistRangeAttack > 5*32){
			if( Squads[0].Marines.size() > 0){
				...
				if( ClosestMarine != NULL && MarineDistClosest > 2*32){
	...
	for(unsigned int i=0; i<Squads[0].BattleCruisers.size(); i++){
		...
		std::set<Unit*> EnemyClose = Squads[0].BattleCruisers[i]->getUnitsInRadius( 15*32 );
		Unit* closest = NULL;
		int closeDist = 9999999;
		BOOST_FOREACH( Unit* close, EnemyClose )
        {
...
void MacroManager::OnFrameZergSquadDefenceChoke(int id){
	...
	*/
	...
	for(unsigned int i=0; i<Squads[0].Goliaths.size(); i++){
		//Broodwar->printf("%s",Marines[i]->getOrder().c_str() );		
		std::set<Unit*> EnemyClose = Squads[0].Goliaths[i]->getUnitsInRadius( 30*32 );
		Unit* closest = NULL;
		int closeDist = 9999999;
		BOOST_FOREACH( Unit* close, EnemyClose )
        {
	...
	for(unsigned int i=0; i<Squads[0].Vultures.size(); i++){
		//Broodwar->printf("%s",Marines[i]->getOrder().c_str() );		
		std::set<Unit*> EnemyClose = Squads[0].Vultures[i]->getUnitsInRadius( 30*32 );
		Unit* closest = NULL;
		int closeDist = 9999999;
		BOOST_FOREACH( Unit* close, EnemyClose )
        {
	...
	for(unsigned int i=0; i<Squads[0].Marines.size(); i++){
		//Broodwar->printf("%s",Marines[i]->getOrder().c_str() );		
		std::set<Unit*> EnemyClose = Squads[0].Marines[i]->getUnitsInRadius( 30*32 );
		Unit* closest = NULL;
		int closeDist = 9999999;
		BOOST_FOREACH( Unit* close, EnemyClose )
        {
			...
			&& Squads[0].Marines[i]->getDistance( WDMan->ToRetreat ) > 2*32 ) )
	  {
		...
		BWTA::Region* theRegion =  BWTA::getRegion( Squads[0].Marines[i]->getPosition() );

		if ( ( Squads[0].Marines[i]->getDistance( WDMan->ToRetreat ) > 20*32 )
			&& ( theRegion != InfoMan->OurMainregion && theRegion != BWTA::getRegion( InfoMan->PosOurNat )  )
	...
	for(unsigned int i=0; i<Squads[0].Tanks.size(); i++){
		...
		BWTA::Region* theRegion =  BWTA::getRegion( Squads[0].Tanks[i]->getPosition() );
	...
	for(unsigned int i=0; i<Squads[0].Wraiths.size(); i++){
		//Broodwar->printf("%s",Marines[i]->getOrder().c_str() );		
		std::set<Unit*> EnemyClose = Squads[0].Wraiths[i]->getUnitsInRadius( 30*32 );
		Unit* closest = NULL;
		int closeDist = 9999999;
		BOOST_FOREACH( Unit* close, EnemyClose )
        {
	...
	for(unsigned int i=0; i<Squads[0].Valkryies.size(); i++){
		//Broodwar->printf("%s",Marines[i]->getOrder().c_str() );		
		std::set<Unit*> EnemyClose = Squads[0].Valkryies[i]->getUnitsInRadius( 30*32 );
		Unit* closest = NULL;
		int closeDist = 9999999;
		BOOST_FOREACH( Unit* close, EnemyClose )
        {
...
void MacroManager::OnFrameZergSquadDefence(int id){
	...
	*/
	...
	for(unsigned int i=0; i<Squads[0].Marines.size(); i++){
		...
		std::set<Unit*> EnemyClose = Squads[0].Marines[i]->getUnitsInRadius( 15*32 );
		Unit* closest = NULL;
		int closeDist = 9999999;
		BOOST_FOREACH( Unit* close, EnemyClose )
        {
	...
	for(unsigned int i=0; i< Squads[0].Medics.size(); i++){
		...
		std::set<Unit*> UnitsClose = Squads[0].Medics[i]->getUnitsInRadius( 9*32 );
		Unit* closest = NULL;
		int closeDist = 9999999;
		BOOST_FOREACH( Unit* close, UnitsClose )
        {
...
void MacroManager::OnFrameZergSquad(){
	...
	for(int i=0; i<Squads.size(); i++){
		if( Squads[i].SQstate == SQ_Defend_Base ){
			...
			if( ZState == Z_Choke_Defend || ZState == Z_Plug_Ramp  || ZState == Z_Expand_Defend){
...
void MacroManager::OnFrameZerg(){
	...
	OnFrameZergSquad();
		...
		// && CurrentStrat != "2 Port Wraith"
		int Frames4Bunker = 3500; //total frames to decide plug ramp or bunker vs early pool
			...
			BWAPI::TilePosition GeyserTile = CCmanager[0]->Geysers[0]->getTilePosition();
			  ...
			  if( CCmanager[0]->LandLocation.x() < GeyserTile.x() - 4 ){ //ref is right
		       BunkerCloseRef = BWAPI::TilePosition ( CCmanager[0]->LandLocation.x() + 6, CCmanager[0]->LandLocation.y()  );
				...
				Broodwar->printf("also adding barracks + academy to build order" );
				/*
	            BO newBO2;
	    ...
	    if( ZState == Z_Init && Broodwar->getFrameCount() > Frames4Bunker ){
			...
			//check if hatchery is build at expansions
			for( int i=0; i<InfoMan->EnemyBuildings.size(); i++ ){
				if( InfoMan->EnemyNatural != BWAPI::Positions::Unknown ){
					if( InfoMan->EnemyBuildings[i].type == BWAPI::UnitTypes::Zerg_Hatchery
						&& InfoMan->EnemyBuildings[i].position.getDistance( BWAPI::Position(InfoMan->EnemyNatural) ) < 5*32){
                            //12 hatch -> can build CC directly at natural
							ZState = Z_Expand_Defend;
					}
				}
		...
		*/
		...
		if( ProdMan->TotalTypeInQueue( BWAPI::UnitTypes::Terran_Academy ) > 0 ){
			MacroMan->PlaceNatBunker();
		}
		...
		*/
		...
		if( ZState == Z_Choke_Defend || ZState == Z_Expand_Defend){
			...
			if( CurrentStrat == "2 rax FE" ){
				...
				if( Squads[0].ScienceVessels.size() == 0  && Broodwar->getFrameCount() > 13000 ){
			    ZState = Z_Push;
			    Squads[0].SQstate = SQ_Attack_Base;
				}
			  ...
			  /*
			  if( Squads[0].ScienceVessels.size() > 0 ){
			    ZState = Z_Push;
			    Squads[0].SQstate = SQ_Attack_Base;
			  }
			  */
...
void MacroManager::OnFrameTerranSquadAttack(int id)
{
	...
	*/
	...
	for(unsigned int i=0; i<Squads[0].Marines.size(); i++){
		...
		std::set<Unit*> EnemyClose = Squads[0].Marines[i]->getUnitsInRadius( 9*32 );
		Unit* closest = NULL;
		int closeDist = 9999999;
		bool Bunker = false;
		BOOST_FOREACH( Unit* close, EnemyClose )
	...
	for(unsigned int i=0; i<Squads[0].Tanks.size(); i++){
		...
		//std::set<Unit*> EnemyClose = Squads[0].Tanks[i]->getUnitsInRadius( MaxDistEAttack );
		Unit* closest = NULL;
		...
		for( int j=0; j<BunkerInfo.size(); j++){
			if(  Squads[0].Tanks[i]->getDistance( BunkerInfo[j] ) <= 12*32  ){
				if( Squads[0].Tanks[i]->getType() == BWAPI::UnitTypes::Terran_Siege_Tank_Tank_Mode){
			     Squads[0].Tanks[i]->useTech( BWAPI::TechTypes::Tank_Siege_Mode);
			     continue;
		       }
	...
	for(unsigned int i=0; i<Squads[0].Vultures.size(); i++){
		...
		//std::set<Unit*> EnemyClose = Squads[0].Vultures[i]->getUnitsInRadius( MaxDistEAttack  );
		Unit* closest = NULL;
		int closeDist = 9999999;
		bool Bunker = false;
		//BOOST_FOREACH( Unit* close, EnemyClose )
	  ...
	  */
	  ...
	  if( BWTA::getRegion( Squads[0].Vultures[i]->getPosition() )  != InfoMan->OurMainregion
		  && BWTA::getRegion( Squads[0].Vultures[i]->getPosition() )  != InfoMan->NatRegion ){
          if(  closest != NULL ){
	...
	for(unsigned int i=0; i<Squads[0].Goliaths.size(); i++){
		...
		int MaxDistEAttack =  15*32;
		//std::set<Unit*> EnemyClose = Squads[0].Goliaths[i]->getUnitsInRadius( MaxDistEAttack );
		Unit* closest = NULL;
		int closeDist = 9999999;
		bool Bunker = false;
	  ...
	  */
	  ...
	  //if the enemies areo ut of weapons reach and outside the main or natural, just keep moving
	  if( BWTA::getRegion( Squads[0].Goliaths[i]->getPosition() )  != InfoMan->OurMainregion
		  && BWTA::getRegion( Squads[0].Goliaths[i]->getPosition() )  != InfoMan->NatRegion ){
          if(  closest != NULL ){
			  if(  ( closest->getType() == BWAPI::UnitTypes::Terran_Vulture || closest->getType() == BWAPI::UnitTypes::Terran_SCV )
				  && closeDist > 6*32 ){
...
void MacroManager::OnFrameTerranSquadDefence(int id)
{
	...
	*/
	...
	for(unsigned int i=0; i<Squads[0].Vultures.size(); i++){
		...
		std::set<Unit*> EnemyClose = Squads[0].Vultures[i]->getUnitsInRadius( 15*32 );
		Unit* closest = NULL;
		int closeDist = 9999999;
		BOOST_FOREACH( Unit* close, EnemyClose )
        {
	  ...
	  // if we can't shoot, run away  if(kite)
		...
		if ( ( closeDist < 5*32 &&  Squads[0].Vultures[i]->getGroundWeaponCooldown() != 0
			...
			||  
			Squads[0].Vultures[i]->getDistance( InfoMan->PosMainChoke ) < 2*32
			||
			BWTA::getRegion( Squads[0].Vultures[i]->getPosition() ) != InfoMan->OurMainregion 
			)
	  {
...
void MacroManager::OnFrameTerran(){
	...
	if( (TState == T_Choke_Defend ||  TState == T_Hold_BBS)
		&& Broodwar->self()->completedUnitCount( BWAPI::UnitTypes::Terran_Vulture) > 1 ){
...
void MacroManager::AddUnit( Unit* unit){
	...
	*/
	...
	if( unit->getType() == BWAPI::UnitTypes::Terran_Bunker){
		...
		if( BWTA::getRegion( unit->getPosition() ) != InfoMan->OurMainregion  ){
			NatBunkerChoke = unit;
		}
