...
void MicroManagerZerg::onFrame()
{
	...
	for(int j=0; j<DefenceBases.size(); j++){
		...
		int MarinesNeeded = 2 + ( defScore / 50.0 );
...
void MicroManagerZerg::InitialPush()
{
	...
	for(unsigned int i=0; i<Squads[0].Marines.size(); i++){
		...
		std::set<Unit*> EnemyClose = Squads[0].Marines[i]->getUnitsInRadius( 10*32 );
		Unit* closest = NULL;
...
void MicroManagerZerg::OnFrameZergSquadDefendExpand( MicroSquadZerg squad ){
	...
	*/
	...
	for(unsigned int i=0; i<squad.Marines.size(); i++){
		...
		std::set<Unit*> EnemyClose = squad.Marines[i]->getUnitsInRadius( MaxDistAttack );
		Unit* closest = NULL;
		int closeDist = 9999999;
		bool Sunken = false;
		int totSunk = 0;
	...
	for(unsigned int i=0; i< squad.Medics.size(); i++){
		...
		std::set<Unit*> UnitsClose = squad.Medics[i]->getUnitsInRadius( 10*32 );
		Unit* closest = NULL;
		int closeDist = 9999999;
		bool Sunken = false;
		int totSunk = 0;
