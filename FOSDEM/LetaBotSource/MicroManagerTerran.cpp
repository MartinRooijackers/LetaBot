	...
	for( int i=0; i<Graph.size(); i++){
		...
		BOOST_FOREACH(BWTA::Region* reg, Graph[i].reg->getReachableRegions()) {
			for(int k=0; k<Graph.size(); k++){
				...
				if (reg->getCenter() == Graph[k].reg->getCenter() 
					&&  Graph[k].reg->getCenter() != Graph[i].reg->getCenter() ) {
					Graph[i].Edges.push_back(k);
				}
			}
		}
		*/
		BOOST_FOREACH(BWTA::Chokepoint* choke, Graph[i].reg->getChokepoints()) {
			for (int k = 0; k<Graph.size(); k++) {
				if (choke->getRegions().first->getCenter() == Graph[k].reg->getCenter()
					&& choke->getRegions().second->getCenter() == Graph[i].reg->getCenter()) {
					Graph[i].Edges.push_back(k);
				}
				if (choke->getRegions().second->getCenter() == Graph[k].reg->getCenter()
					&& choke->getRegions().first->getCenter() == Graph[i].reg->getCenter()) {
					Graph[i].Edges.push_back(k);
				}
...
void MicroManagerTerran::drawInfo()
{
		...
		*/
		...
		BOOST_FOREACH(int adjacent, Graph[i].Edges) {
			Broodwar->drawLineMap(Graph[i].Center.x , Graph[i].Center.y, Graph[adjacent].Center.x, Graph[adjacent].Center.y, BWAPI::Colors::White);
		}
...
void MicroManagerTerran::OnFrameTerranSquadDefendExpand(  MicroSquadTerran squad  )
{
	...
	for(unsigned int i=0; i<squad.Vultures.size(); i++){
		...
		Unitset EnemyClose = squad.Vultures[i]->getUnitsInRadius( 9*32 );
		Unit closest = NULL;
	...
	for(unsigned int i=0; i<squad.Goliaths.size(); i++){
		...
		Unitset EnemyClose = squad.Goliaths[i]->getUnitsInRadius( 15*32 );
		Unit closest = NULL;
		int closeDist = 9999999;
		bool Bunker = false;
		BOOST_FOREACH( Unit close, EnemyClose )
