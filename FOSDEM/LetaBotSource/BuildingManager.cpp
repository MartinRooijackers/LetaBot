...
BuildingManager::BuildingManager(){
	...
	BOOST_FOREACH(Unit m, Broodwar->getMinerals()) {
	...
	//Block gas locations
	BOOST_FOREACH(Unit g, Broodwar->getGeysers()) {
	...
	if( type == BWAPI::UnitTypes::Terran_Refinery ){
		...
		for( int i=0; i<CCmanager.size(); i++){
			if( CCmanager[i]->Geysers.size() > 0
				...
				return CCmanager[i]->Geysers[0]->getInitialTilePosition();
			}
		}

		Broodwar->printf("No Geyser found");
	...
	for(int x=xPos; x<xPos+ SpaceArray[BuildType].width; x++ ){
		for(int y=yPos; y<yPos+ SpaceArray[BuildType].heigth; y++ ){
		   ...
		   if( BuildType == 2 ){//Barracks gets put down first
			   NextToB = true;//
			   continue;//since barracks is put down first, no need for adjacency checks
		   }
		   //Check if the building is next to another building
	...
	ReadWallData();
	  ...
	  for(int x = TopLeft.x; x<BottomRight.x; x++){
	    for(int y = TopLeft.y; y<BottomRight.y; y++){
			...
			double getDist = CommandC.getApproxDistance(Current);//  BWTA::getGroundDistance(CommandC, Current);
			if( getDist < MinT ){
				MinT = getDist;
				Closest = Current;
			}
...
// do something
	  ...
	  //for paper: put the result in a .txt file
	  std::ofstream foutTime("bwapi-data/write/wallTime.txt", std::fstream::app);//, std::fstream::app 
	  if (!foutTime.fail()) {
		  foutTime << timediff  << "\n";
	  }
