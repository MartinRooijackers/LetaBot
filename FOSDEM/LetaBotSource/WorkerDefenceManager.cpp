...
void WorkerDefenceManager::OnFrame()
{
	...
	BOOST_FOREACH(BWAPI::Unit  enemy,  Broodwar->enemy()->getUnits() ) {
		...
		if( enemy->getDistance( CCpos ) < 40*32 
			...
			&& BWTA::getRegion( enemy->getTilePosition() ) == InfoMan->OurMainregion){
			getAWorker++;
			toAttack = enemy;
		}
		if( enemy->getDistance( CCpos ) < 9*32 
			&& !enemy->getType().isFlyer()
			&& !enemy->getType().isWorker()
		...
		if( (enemy->getDistance( CCpos ) < 80*32)
			...
			&&  enemy->getType() == BWAPI::UnitTypes::Terran_Marine){
				if( MacroMan->Squads[0].Bunkers.size() == 0 ){
					getAWorker += 2;
				} else if( BWTA::getRegion( enemy->getTilePosition() ) == InfoMan->OurMainregion ){
			       getAWorker += 2;
				}
			//getAWorker++;
			//if( MacroMan->ZState == Z_Pool_Defend){
		...
		if( enemy->getType().isBuilding()
			...
			&& 
			(BWTA::getRegion( enemy->getTilePosition() ) == InfoMan->OurMainregion
			|| BWTA::getRegion( enemy->getTilePosition() ) == InfoMan->NatRegion  )
                            ){

			bool checkWallProtoss = false; //if we got a wall, we are already protected
			if (Broodwar->enemy()->getRace() == BWAPI::Races::Protoss
				&& bManager->WallSound == true
				&& BWTA::getRegion(enemy->getTilePosition()) == InfoMan->NatRegion) {
				checkWallProtoss = true;
			}
			...
			//only pull workers if we don't use a wall for enemy buildings in the natural
			if (Broodwar->enemy()->getRace() == BWAPI::Races::Protoss
				&& checkWallProtoss == false) {
				getAWorker += 5;
			}
			//getAWorker++;
			//if( MacroMan->ZState == Z_Pool_Defend){
	...
	for(unsigned int i=0; i<DefenceSCVS.size(); i++){
		...
		if( toAttack != NULL ){
		  ...
		  Unitset EnemyClose = DefenceSCVS[i]->getUnitsInRadius( 30*32 );
		  Unit closest = NULL;
