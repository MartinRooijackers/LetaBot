...
void ScoutManager::ScoutMain( Unit scv )
{
	...
	if( BWTA::getRegion( scv->getTilePosition() ) != BWTA::getRegion(BWAPI::TilePosition(InfoMan->PosEnemyBase )) ){
	...
	if (Xlocation > Broodwar->mapWidth()*32 ) {
		Xlocation = Broodwar->mapWidth() * 32 - 1;
	}
	if (Ylocation > Broodwar->mapHeight() * 32) {
		Ylocation = Broodwar->mapHeight() * 32 - 1;
	}
