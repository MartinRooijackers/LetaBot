...
InformationManager::InformationManager() 
{
	...
	OurMainregion = BWTA::getRegion(BWAPI::TilePosition(PosOurBase));
	...
	NatRegion = BWTA::getRegion(BWAPI::TilePosition(PosOurNat));
	...
	*/
	...
	for (auto& gas : Broodwar->getGeysers()) {
		GeyserLocations.push_back((gas)->getTilePosition());
	}
	/*
	for(UnitSet::iterator g = Broodwar->getGeysers().begin(); g != Broodwar->getGeysers().end(); ++g) {
	...
	*/
	...
	BWTA::Region* natRegion = BWTA::getRegion(BWAPI::TilePosition(PosOurNat ));
	BWTA::Region* mainRegion = BWTA::getRegion(BWAPI::TilePosition(PosOurBase) );
   ...
   if( BunkerChoke == NULL && mainRegion != NULL && natRegion != NULL ){
       ...
       BOOST_FOREACH( BWTA::Chokepoint*  choke, natRegion->getChokepoints() ){
		   double distance = BWAPI::TilePosition(choke->getCenter()).getApproxDistance(OurBase);
		   if(distance < closestNat ){
			   BunkerChoke = choke;
			   closestNat = distance;
		   }
	   }
